...
/// Section : height problem fixed code 

// Section : Weight History - gap fix (scaleX-only painter)
import 'dart:convert';
import 'dart:ui' as ui;
...
class WeightTimelineInfinite extends StatelessWidget {
  ...
  static const double _originalWidth = 290.0;
  static const double _entrySpacing  = 84.0;
  static const double _firstCircleY  = 47.5;
  static const double _circleRadius  = 38.5;
  ...
  Widget build(BuildContext context) {
    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        // Scale ONLY by width — this is the single source of truth.
        final double scale = constraints.maxWidth / _originalWidth;

        final int    n           = entries.length;
        final double lastCircleY = _firstCircleY + (n - 1) * _entrySpacing;

        // Bottom-most logical Y in the 290-wide coordinate space.
        final double logicalBottom;
        if (n == 1) {
          logicalBottom = lastCircleY + _circleRadius + 40;
        } else {
          final double prevCircleY = lastCircleY - _entrySpacing;
          // End path bottom sits at prevCircleY + 128 at most; add padding.
          logicalBottom = prevCircleY + 128 + 48;
        }

        // Rendered pixel height = logical units × scale.
        final double renderedHeight = logicalBottom * scale;
...
class WeightTimelineInfinitePainter extends CustomPainter {
  ...
  @override
  void paint(ui.Canvas canvas, ui.Size size) {
    // KEY FIX: scale only by width — no height-based scaling.
    // Previously using min(scaleX, scaleY) caused a large offsetY that pushed
    // all content far down, creating the blank gap below the title text.
    final double scale = size.width / _originalWidth;

    canvas.save();
    canvas.scale(scale, scale); // uniform scale anchored at top-left (0,0)
    ...
    switch (designType) {
      ...
      case DesignType.end:
        if (_isEndOnLeft(totalEntries)) {
          ...
          path.lineTo(270, prevCircleY + 124);
        } else {
          path.moveTo(152, prevCircleY + 42);
          path.lineTo(227, prevCircleY + 42);
          path.cubicTo(250.748, prevCircleY + 42, 270, prevCircleY + 61.252, 270, prevCircleY + 85);
