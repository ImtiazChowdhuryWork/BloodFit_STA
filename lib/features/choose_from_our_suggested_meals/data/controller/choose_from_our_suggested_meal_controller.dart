...
class ChooseFromOurSuggestedMealController extends GetxController {
  ...
  ///--------->>> Section : Selected Meal Data Models for Review Screen
  
  /// Model to hold complete meal data for review screen
  Map<String, Map<String, dynamic>> getSelectedMealsCompleteData() {
    LoggerUtils.debug("╔═══════════════════════════════════════════════════════════");
    LoggerUtils.debug("📦 [REVIEW] Getting complete selected meal data for review screen");
    LoggerUtils.debug("╚═══════════════════════════════════════════════════════════");
    
    Map<String, Map<String, dynamic>> selectedMealsData = {};
    
    // Helper function to get meal data by ID and type
    Map<String, dynamic>? getMealDataById(String mealId, String mealType) {
      if (mealId.isEmpty) return null;
      
      Datum? meal;
      if (mealType == 'breakfast') {
        meal = breakfastRecentChosenMeals.firstWhere(
          (m) => m.id == mealId,
          orElse: () => Datum(),
        );
      } else if (mealType == 'lunch') {
        meal = lunchRecentChosenMeals.firstWhere(
          (m) => m.id == mealId,
          orElse: () => Datum(),
        );
      } else if (mealType == 'dinner') {
        meal = dinnerRecentChosenMeals.firstWhere(
          (m) => m.id == mealId,
          orElse: () => Datum(),
        );
      }
      ...
      if (meal != null && meal.id != null) {
        LoggerUtils.debug("✅ [REVIEW] Found previously selected meal: ${meal.mealName}");
        return {
          'mealName': meal.mealName ?? '',
          'description': meal.description ?? '',
          'kcal': meal.kcal ?? 0,
          'ingredients': meal.ingredients ?? [],
          'caloryCount': meal.caloryCount ?? [],
          'image': meal.image ?? '',
          'mealType': mealType,
          'hasId': true,
          'id': meal.id,
    ...
    // Helper function to get AI meal data by index
    Map<String, dynamic>? getAiMealData(String mealId, String mealType) {
      if (!mealId.contains('_')) return null;
    ...
    // Get breakfast meal data
    if (selectedBreakfastMealId.value.isNotEmpty) {
      final mealId = selectedBreakfastMealId.value;
      Map<String, dynamic>? mealData;
      
      if (mealId.contains('_')) {
        mealData = getAiMealData(mealId, 'breakfast');
      } else {
        mealData = getMealDataById(mealId, 'breakfast');
      }
      
      if (mealData != null) {
        selectedMealsData['breakfast'] = mealData;
      }
    }
    
    // Get lunch meal data
    if (selectedLunchMealId.value.isNotEmpty) {
      final mealId = selectedLunchMealId.value;
      Map<String, dynamic>? mealData;
      
      if (mealId.contains('_')) {
        mealData = getAiMealData(mealId, 'lunch');
      } else {
        mealData = getMealDataById(mealId, 'lunch');
      }
      
      if (mealData != null) {
        selectedMealsData['lunch'] = mealData;
      }
    }
    
    // Get dinner meal data
    if (selectedDinnerMealId.value.isNotEmpty) {
      final mealId = selectedDinnerMealId.value;
      Map<String, dynamic>? mealData;
      
      if (mealId.contains('_')) {
        mealData = getAiMealData(mealId, 'dinner');
      } else {
        mealData = getMealDataById(mealId, 'dinner');
      }
      
      if (mealData != null) {
        selectedMealsData['dinner'] = mealData;
      }
    }
    
    LoggerUtils.debug("📦 [REVIEW] Total meals retrieved: ${selectedMealsData.length}");
    LoggerUtils.debug("📦 [REVIEW] Breakfast: ${selectedMealsData.containsKey('breakfast') ? '✅' : '❌'}");
    LoggerUtils.debug("📦 [REVIEW] Lunch: ${selectedMealsData.containsKey('lunch') ? '✅' : '❌'}");
    LoggerUtils.debug("📦 [REVIEW] Dinner: ${selectedMealsData.containsKey('dinner') ? '✅' : '❌'}");
...
Map<String, dynamic> buildMealPlanData() {
      ...
      if (parts.length >= 3) {
        ...
        if (index >= 0) {
          ...
          if (type == 'breakfast') {
            ...
            if (index < categoryList.length) {
              meal = categoryList[index];
              baseImageUrl = breakFastMealImage;
            }
          } else if (type == 'lunch') {
            final categoryList = _getLunchCategoryList(category);
            if (index < categoryList.length) {
              meal = categoryList[index];
              baseImageUrl = lunchMealImage;
            }
          } else if (type == 'dinner') {
            final categoryList = _getDinnerCategoryList(category);
            if (index < categoryList.length) {
              meal = categoryList[index];
              baseImageUrl = dinnerMealImage;
            }
          }
  ...
  if (selectedBreakfastMealId.value.isNotEmpty) {
    final mealData = buildMealData(selectedBreakfastMealId.value, 'breakfast');
    if (mealData != null) {
      mealPlanData['meals']['breakfast'] = [mealData];
      // Convert base64 image to URL if needed
      mealPlanData['breakfastImage'] = _convertImageToUrl(breakFastMealImage);
  ...
  if (selectedLunchMealId.value.isNotEmpty) {
    final mealData = buildMealData(selectedLunchMealId.value, 'lunch');
    if (mealData != null) {
      mealPlanData['meals']['lunch'] = [mealData];
      // Convert base64 image to URL if needed
      mealPlanData['lunchImage'] = _convertImageToUrl(lunchMealImage);
  ...
  if (selectedDinnerMealId.value.isNotEmpty) {
    final mealData = buildMealData(selectedDinnerMealId.value, 'dinner');
    if (mealData != null) {
      mealPlanData['meals']['dinner'] = [mealData];
      // Convert base64 image to URL if needed
      mealPlanData['dinnerImage'] = _convertImageToUrl(dinnerMealImage);
  ...
  // Log each meal's image field
  if (mealPlanData['meals']['breakfast'] != null) {
    final breakfast = (mealPlanData['meals']['breakfast'] as List).first as Map;
    LoggerUtils.debug("   • Breakfast Image (in meal): ${breakfast['image'] ?? 'NOT INCLUDED'}");
  }
  if (mealPlanData['meals']['lunch'] != null) {
    final lunch = (mealPlanData['meals']['lunch'] as List).first as Map;
    LoggerUtils.debug("   • Lunch Image (in meal): ${lunch['image'] ?? 'NOT INCLUDED'}");
  }
  if (mealPlanData['meals']['dinner'] != null) {
    final dinner = (mealPlanData['meals']['dinner'] as List).first as Map;
    LoggerUtils.debug("   • Dinner Image (in meal): ${dinner['image'] ?? 'NOT INCLUDED'}");
  }
  
  // Add top-level image fields for backend compatibility
  if (mealPlanData['meals']['breakfast'] != null) {
    final breakfast = (mealPlanData['meals']['breakfast'] as List).first as Map;
    mealPlanData['breakfastImage'] = breakfast['image'] ?? 'https://faisal5000.merinasib.shop/images/cucumber.jpeg';
  }
  if (mealPlanData['meals']['lunch'] != null) {
    final lunch = (mealPlanData['meals']['lunch'] as List).first as Map;
    mealPlanData['lunchImage'] = lunch['image'] ?? 'https://faisal5000.merinasib.shop/images/cucumber.jpeg';
  }
  if (mealPlanData['meals']['dinner'] != null) {
    final dinner = (mealPlanData['meals']['dinner'] as List).first as Map;
    mealPlanData['dinnerImage'] = dinner['image'] ?? 'https://faisal5000.merinasib.shop/images/cucumber.jpeg';
  }
  
  LoggerUtils.debug("   • breakfastImage (top-level): ${mealPlanData['breakfastImage']}");
  LoggerUtils.debug("   • lunchImage (top-level): ${mealPlanData['lunchImage']}");
  LoggerUtils.debug("   • dinnerImage (top-level): ${mealPlanData['dinnerImage']}");
